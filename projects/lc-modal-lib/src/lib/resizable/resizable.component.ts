...
@Component({
  selector: 'resizable',
  template: `
        <div class="resizable-handle resizable-right"
            (mousedown)="onMousedown($event, 'right')"
            (touchstart)="onMousedown($event, 'right')"></div>
        <div class="resizable-handle resizable-bottom"
            (mousedown)="onMousedown($event, 'bottom')"
            (touchstart)="onMousedown($event, 'bottom')"></div>
        <div class="resizable-handle resizable-corner"
            (mousedown)="onMousedown($event, 'both')"
            (touchstart)="onMousedown($event, 'both')"><span></span></div>
    `,
...
export class Resizable implements AfterViewInit, OnDestroy {
  ...
  public ngAfterViewInit() {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      const mouseUpFn = this.renderer.listen(
        'document',
        'mouseup',
        (event: MouseEvent) => this.onMouseUp()
      );
  ...
  private calcNewSize(event: MouseEvent) {
    ...
    if (this.resizeDirection == 'right' || this.resizeDirection == 'both') {
      this.width = Math.max(
        mousePos.x - this.parent.getPositionLeft(),
        this.parent.getMinWidth()
      );
    }

    if (this.resizeDirection == 'bottom' || this.resizeDirection == 'both') {
      this.height = Math.max(
        mousePos.y - this.parent.getPositionTop(),
        this.parent.getMinHeight()
      );
    }
